...
T_list = np.arange(5000, 7001, 1000)
cmap = plt.cm.rainbow
...
def main():
    ...
    for path, nm, ls, lw, mk in zip(QHA_path, QHA_name, linestyles, linewidth, markerstyle):
        ...
        for T in T_list:
            ...
            first_curve = False
            ax.plot(P, Fvib, linestyle=ls, marker=mk, color=color, linewidth=lw, label=label, markersize=8)

    #=============== plot ==================#
    ax.set_xlim(230, 500)
    ax.set_ylim(-5.3, -2.3)
    ax.set_xlabel("P (GPa)", fontsize=16)
    ax.set_ylabel("Fvib (eV/atom)", fontsize=16)
